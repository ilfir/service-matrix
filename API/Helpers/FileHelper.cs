...
public class FileHelper : IFileHelper
{
    ...
    public async Task<IEnumerable<string>> ReadFileAsync(string directory, string fileName)
      {
        ...
        if (!File.Exists(filePath))
          ...
          _logger?.LogDebug("Reading file asynchronously: {FilePath}", filePath);
      ...
      /// <summary>
      /// Reads all lines from a file. (Deprecated: use ReadFileAsync instead.)
      /// </summary>
      /// <param name="directory">The base directory relative to the app.</param>
      /// <param name="fileName">The file name.</param>
      /// <returns>The lines from the file, or an empty array if not found.</returns>
    ...
    public async Task WriteFileAppend(string directory, string fileName, IEnumerable<string> newContents)
      ...
      /// <summary>
      /// Reads all lines from a file asynchronously using the default FileHelper instance.
      /// </summary>
      /// <param name="directory">The base directory relative to the app.</param>
      /// <param name="fileName">The file name.</param>
      /// <returns>The lines from the file, or an empty array if not found.</returns>
    public static Task<IEnumerable<string>> ReadFileAsyncStatic(string directory, string fileName)
      {
        var instance = new FileHelper();
        return instance.ReadFileAsync(directory, fileName);
      }
      ...
      /// <param name="fileName">The file name.</param>
      /// <returns>The lines from the file, or an empty array if not found.</returns>
    public static IEnumerable<string> ReadFileSync(string directory, string fileName)
      {
        var instance = new FileHelper();
